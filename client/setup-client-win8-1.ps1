<#
.SYNOPSIS
    SMBProxy - Windows 8 / 8.1 (v4.5 - 双模式)
.DESCRIPTION
...
# --- 系统检测: Windows 8 / 8.1 (版本 6.2/6.3, 工作站) ---
$os = Get-CimInstance Win32_OperatingSystem
if (-not ($os.Version -like "6.2.*" -or $os.Version -like "6.3.*") -or $os.ProductType -ne 1) {
    Write-Host "[错误] 本脚本仅适用于 Windows 8 / 8.1, 当前: $($os.Caption)" -ForegroundColor Red; pause; exit 1
}
Write-Host "[系统] $($os.Caption) — Win8/8.1 模式" -ForegroundColor Cyan
...
$TaskName = "SMBProxy_Engine"; $LocalPort = 445; $FwPrefix = "SMBProxy-W8"
...
function Setup-Task {
    # Win8/8.1 用 schtasks (兼容性最好); 开机延迟 5 秒等网络栈就绪
