<#
.SYNOPSIS
    SMBProxy - Windows Server 2008 R2 (v4.5 - 双模式)
.DESCRIPTION
    ...
    注意: 2008R2 需 WMF 5.1 + .NET 4.8 + SHA-2 补丁, 脚本自动检测并从 2008r2plug 安装。
#>
...
$TaskName = "SMBProxy_Engine"; $LocalPort = 445; $FwPrefix = "SMBProxy-S2008"
$ModePath = "$ToolboxDir\mode.txt"

$os = Get-WmiObject Win32_OperatingSystem
if (-not ($os.Version -like "6.1.*") -or $os.ProductType -eq 1) {
    Write-Host "[错误] 本脚本仅适用于 Windows Server 2008 R2, 当前: $($os.Caption)" -ForegroundColor Red; pause; exit 1
}
Write-Host "[系统] $($os.Caption) — Server 2008 R2 模式" -ForegroundColor Cyan
...
function Show-Menu {
    ...
    Write-Host "     SMBProxy - Server 2008 R2 (v4.5)                   " -ForegroundColor Cyan
    Write-Host "====================================================" -ForegroundColor Cyan
    $mode = Get-Mode
