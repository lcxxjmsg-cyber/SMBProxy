<#
.SYNOPSIS
     SMB 端口转发 - Windows Server 2012 / 2012 R2 (v3.1-server)
.DESCRIPTION
      与 setup-client.ps1 (Win10/11 版) 功能相同。NetAdapter / 防火墙 / CIM 模块
      与 Win10 一致, 计划任务改用 schtasks 方案 (兼容性考虑)。
      核心引擎完全一致。
     [Windows Server 2016+ 请使用 setup-server-2016plus.ps1]
     [Windows Server 2008 R2 请使用 setup-server-2008r2.ps1]
#>
...
$isServer2012 = ($os.Version -like "6.2.*" -or $os.Version -like "6.3.*")
if (-not $isServer2012) {
    Write-Host "====================================================" -ForegroundColor Red
    Write-Host " [错误] 此脚本仅适用于 Windows Server 2012 / 2012 R2" -ForegroundColor Red
    ...
    Write-Host "  Server 2008 R2 请使用 setup-server-2008r2.ps1" -ForegroundColor Yellow
    Write-Host "====================================================" -ForegroundColor Red
    pause
    exit 1
}
Write-Host "[系统] 检测到 $($os.Caption) — 使用 Server 2012 模式" -ForegroundColor Cyan
...
function Show-Menu {
    ...
    Write-Host "     SMB 端口转发 - Server 菜单 (v3.1-Srv12)       " -ForegroundColor Cyan
    Write-Host "====================================================" -ForegroundColor Cyan
