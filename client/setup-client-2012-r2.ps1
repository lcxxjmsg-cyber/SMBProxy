<#
.SYNOPSIS
    SMBProxy - Windows Server 2012 / 2012 R2 (v4.5 - 双模式)
.DESCRIPTION
...
# --- 系统检测: Windows Server 2012 / 2012 R2 (版本 6.2/6.3, 服务器) ---
$os = Get-CimInstance Win32_OperatingSystem
if (-not ($os.Version -like "6.2.*" -or $os.Version -like "6.3.*") -or $os.ProductType -eq 1) {
    Write-Host "[错误] 本脚本仅适用于 Windows Server 2012 / 2012 R2, 当前: $($os.Caption)" -ForegroundColor Red; pause; exit 1
}
Write-Host "[系统] $($os.Caption) — Server 2012/R2 模式" -ForegroundColor Cyan
...
$TaskName = "SMBProxy_Engine"; $LocalPort = 445; $FwPrefix = "SMBProxy-S2012"
...
function Setup-Task {
    # Server 2012/R2 用 schtasks; 开机延迟 5 秒等网络栈就绪
...
function Show-Menu {
    ...
    Write-Host "     SMBProxy - Server 2012/R2 (v4.5)                 " -ForegroundColor Cyan
    Write-Host "====================================================" -ForegroundColor Cyan
    $mode = Get-Mode
