<#
.SYNOPSIS
    SMBProxy - Server 2012/2012 R2 (v4.5 - 一端一网卡版)
.DESCRIPTION
...
if (-not ($os.Version -like "6.2.*" -or $os.Version -like "6.3.*")) {
    Write-Host "[错误] 仅适用于 Windows Server 2012 / 2012 R2, 当前: $($os.Caption)" -ForegroundColor Red; pause; exit 1
}
Write-Host "[系统] $($os.Caption) — Server 2012 模式" -ForegroundColor Cyan
...
function Show-Menu {
    ...
    Write-Host "     SMBProxy - Server 2012/2012 R2 (v4.5)          " -ForegroundColor Cyan
    Write-Host "====================================================" -ForegroundColor Cyan
